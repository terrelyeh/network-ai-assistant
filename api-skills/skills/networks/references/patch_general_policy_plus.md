Update general policy settings of a specific network.

### Request Body Example

> **Schema illustration only. Values are fictional. Do NOT use as PATCH body base.**
> Always use the actual GET response as the base and overwrite only the fields the user requested.
...
```json
{
  ...
  "compliance_record": {
    "legal_name": "requester full name",
    "datetime": 1640172640000
  },
  "custom_ntp_server": {
    "is_enable": false,
    "host": "pool.ntp.org",
...
| compliance_record                 | object        | Compliance record                                  |
...
### compliance_record Schema (PATCH only)


| Field      | Type    | Description                           |
| ---------- | ------- | ------------------------------------- |
| legal_name | string  | Legal name in compliance record.      |
| datetime   | integer | Current UTC in milliseconds (e.g. `1640172640000`). |


### Input Validation Notes
- Only update fields explicitly requested by user.
- Keep unrelated fields unchanged to avoid accidental resets.
...
- Call this API exactly once per confirmed update request.
- After success, call GET again for post-update verification.
